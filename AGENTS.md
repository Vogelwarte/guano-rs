...
Notes for agents working on `guano`. Read the "GUANO rules that bite" section before touching
...
|---|---|---|---|
| `cli` | yes | `clap`, `serde_json` | the `guano` binary, via `required-features` on `[[bin]]` |
...
```sh
cargo test -p guano                                          # 27 tests + 9 doc tests
cargo test -p guano --no-default-features                    # 18 tests + 5 doc tests
cargo test -p guano --no-default-features --features serde   # 27 tests + 9 doc tests
```
...
| Path | What |
|---|---|
| [guano/src/lib.rs](guano/src/lib.rs) | The whole library: `GuanoFile`, `GuanoValue`, `GuanoError`. RIFF walk in `load()`, metadata parse in `parse()`, serde impls after the `Index` impl. |
| [guano/src/main.rs](guano/src/main.rs) | `guano` CLI (clap). `--format kv\|json`, `--compact`. |
...
| [guano/testdata/](guano/testdata/) | Fixtures — see Testing. |
...
Fixtures in [guano/testdata/](guano/testdata/):
...
`weird.wav` is generated by [generate_weird.py](guano/testdata/generate_weird.py) — **edit the
...
**Order matters and is not optional:** `guano-wasm` depends on `guano` by version, so
`cargo package -p guano-wasm` fails with "no matching package named `guano`" until that exact
version is live on crates.io. Publish `guano` first, wait for the index, then `guano-wasm`.

```sh
cargo publish -p guano
cargo publish -p guano-wasm   # only once guano <version> is on crates.io
```

Rules that bite here:

- The `guano` dependency in `guano-wasm/Cargo.toml` carries **both** `path` and `version`. Cargo
...
- **The two versions must move together.** Bump `guano`, and you must bump both the `version` in
  `guano-wasm/Cargo.toml` *and* the `version` in its `guano` dependency line.
...
- `guano` needs its **own** `LICENSE` file: cargo only packages files under the crate directory,
